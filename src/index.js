...
const createWindow = () => {
    ...
    ipcMain.on("obsConnect", (event, data) => {
        ...
        server.on('message', (msg) => {
            ...
            //Set Capture Mouse
            else if (msg[0].includes('toggleCaptureMouse')) {
                console.log(`OSC IN: ${msg}`)
                logEverywhere(`OSC IN: ${msg}`)
                var msgArray = msg[0].split("/")
                msgArray.shift()
                obs.call("GetInputSettings", {
                    ...
                    cursorSetting = !data.inputSettings.cursor
                    obs.call("SetInputSettings", {
                        'inputName': msgArray[0].split('_').join(' '),
                        'inputSettings': {
                        ...
                        console.log("Error: Invalid Syntax. Make Sure There Are NO SPACES in Source Name and Filter name. [Source_Name]/toggleCaptureMouse, example: /SceneCapture1/toggleCaptureMouse")
                        logEverywhere(`Error: Invalid Syntax. Make Sure There Are NO SPACES in Source Name and Filter name. [Source_Name]/toggleCaptureMouse, example: /SceneCapture1/toggleCaptureMouse'\nOSC Recieved: ${msg}`)
                    })
                })
            }
